...
> **検証ベータ（2026-09時点）の限定**: 最初の課題を検証する対象は**大衆向け・個人単位・中堅層**（[`know-to-support-verification/prd.md`](../discussions/prd/know-to-support-verification/prd.md) §2-5）。beatfolio は「アーティストが自分を定義し、外の人に翻訳され、そこから告知が流れ出す**公式の起点**」（同 §2-4）として位置づける。
...
| Support    | 応援に至ったか（検証ベータでは**来場**を定義とする） | オファー経由の来場（SNSリンククリックは先行指標に降格。[`know-to-support-verification/prd.md`](../discussions/prd/know-to-support-verification/prd.md) §6） |
