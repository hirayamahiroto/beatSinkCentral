...
> **ドラフト（凍結前）**。§5 の協議点をユーザーが判断した時点で凍結し、以後の実装はこの表を契約とする。
...
1. **翻訳に API を作らない**（運営 SQL 運用）で確定してよいか — PRD F3 の明記どおりだが、書き込み手順が属人化する
2. **認証不要 write の防御水準** — events / subscriptions の bot・乱用対策をベータでどこまでやるか（最小: bot UA 除去＋サイズ制限。rate limit はベータ規模では見送り、を提案）
3. **subscriptions のパスキー** — 認証不要 write が `artistId` を受ける形は、ガイドライン「ID だけで叩ける非認証エンドポイントを作らない」との整合確認が要る（**作成専用で読み出しが無い**ため許容、を提案。規範側に一文追記）
4. **`GET /story-questions` マスタ API の要否** — 公開 read に文言を埋め込むだけでも足りる（ベータは問い 3 つ固定）。汎用 API はマスタ規範に忠実だが 1 本増える（**ベータでは埋め込み・語彙が動き始めたら API 化**、を提案）
