...
| オファー   | オファーを差し替える  | `POST /artists/:artistId/offers`                     | `{ date, place, ticketUrl, comment, coPerformers[] }`（`coPerformers[]` は `{ name, handle }` を最大 20 件。`handle` は登録済み共演者のみ。`date` は日本時間の今日以降に限る） |
...
- オファーは `ArtistProfile` とは別の集約（`artists` 直下・時限で差し替わる）のため、`profile` の**兄弟**として `offer` を返す。開催日当日（日本時間）の終わりまで有効なオファーだけを載せ、無ければ `null`。開催日を過ぎた行は DB に残るが応答には現れない（本人向け `GET /artists/:artistId/profile` で実装済み）。
