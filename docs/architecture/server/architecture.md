...
| ルール                               | 検出する形                                                                                        | 適用範囲                                                                            |
| ------------------------------------ | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `local/usecase-capability-boundary`  | `usecases/` から `infrastructure/`・`database`・`drizzle-orm`・`@supabase/*` への import          | `src/usecases/**`（経路モジュール・テストを含む全体）                               |
| `local/usecase-capability-parameter` | エクスポート関数の第1引数が権能型（`*Caps` / `*Capabilities`、`Pick` 等で包んだものを含む）でない | `src/usecases/**`（`authorization/` `capabilities/` `testDoubles/` とテストは除外） |
