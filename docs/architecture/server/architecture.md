...
> 旧 `me` 系（`POST /api/artists/me`・`GET|POST /api/artists/me/profile`・`POST /api/artists/me/profile/publish|image`）はクライアント移行が完了するまで併存させ、その後削除する（[api-design-guidelines.md](./api-design-guidelines.md) のリソースアドレッシング参照）。
