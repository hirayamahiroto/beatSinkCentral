...
| ドメインコンテキスト（handle 等）の保持     | ✅ `logFields` → `context` に型付きで宣言        | ◎                  |
...
| ドメインコンテキスト（handle 等） | 必要な分だけ | ○        | 影響範囲の特定に必要                                    |
...
| -------------------- | ---------------------- |
| `.use("*")` の中     | `/api/*`               |
| ハンドラ / `onError` | `/api/artists/:handle` |
...
SLO が悪化したとき、「どの顧客セグメント / どのテナントで起きているか」を特定する必要がある。`logFields` で `handle` / `tenantId` 等のドメイン語彙を宣言しておけば、`context.*` として Datadog で次元別にグルーピングでき、影響範囲を即座に可視化できる。
