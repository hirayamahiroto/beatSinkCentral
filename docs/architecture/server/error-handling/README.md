...
| [implementation.md](./implementation.md) | どう書くか（co-location / Result と throw / errorMap / 追加手順） | 実装者 |
...
- 失敗は **`Result<T, E>` で返す**（VO / usecase）。throw が残るのは返り値の経路を持たないミドルウェア・バリデータ・想定外の例外だけ
