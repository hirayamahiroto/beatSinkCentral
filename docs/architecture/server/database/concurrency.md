...
2. **トランザクション境界の外で `err` に戻す**: Drizzle の `transaction` は throw でしかロールバックしないため、例外はトランザクションの外まで抜けさせる。境界を張るヘルパ（`withWriteCapabilities` / `withRegistrationCapabilities`）が型ガードで判別して `err` を返す。usecase 側に `try/catch` は置かない
...
| ------------------- | -------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------- |
| `createUser`        | LWW            | DB 一意制約で拒否（409） | 新規作成のみ。事前 SELECT は usecase、一意制約違反は `withRegistrationCapabilities` が `AccountIdAlreadyTakenError` へ寄せる |
| `updateMyEmail`     | LWW            | 対象外                   | 自分の email を自分で変更。競合確率低                                                                                        |
| `updateMyAccountId` | LWW            | DB 一意制約で拒否（409） | 自分の accountId を自分で変更。事前 SELECT は usecase、一意制約違反は `withWriteCapabilities` が寄せる                       |
