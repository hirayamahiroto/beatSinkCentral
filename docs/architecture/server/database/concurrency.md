...
2. **トランザクション境界の外で `err` に戻す**: Drizzle の `transaction` は throw でしかロールバックしないため、例外はトランザクションの外まで抜けさせる。capabilities 経路では境界を張るヘルパ（例: `withRegistrationCapabilities`）が、未移行の usecase では `txRunner.run` を包む `try/catch` が、型ガードで判別して `err` を返す
