...
2. **トランザクション境界の外で `err` に戻す**: Drizzle の `transaction` は throw でしかロールバックしないため、例外はトランザクションの外まで抜けさせる。境界を張るヘルパ（`withUserWriteCapabilities` / `withWriteCapabilities` / `withRegistrationCapabilities`）が型ガードで判別して `err` を返す。**判別する型はその権能で書ける範囲に一致させる**（`withUserWriteCapabilities` は `users` しか書けないため `EmailAlreadyTakenError` のみ）。usecase 側に `try/catch` は置かない
...
| `updateMyAccountId` | LWW            | `accountId` を DB 一意制約で拒否（409）           | 自分の accountId を自分で変更。事前 SELECT は usecase、一意制約違反は `withWriteCapabilities` が寄せる                                                |
