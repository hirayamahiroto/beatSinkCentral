...
提案した3手法は production へ取り込み済み。検討用の隔離サンプルは
「取り込み済みの提案が残っていると現在地が読めない」ため削除した。

| 提案手法                                   | production の状況                  | 実装の在り処                                     |
| ------------------------------------------ | ---------------------------------- | ------------------------------------------------ |
| ブランド型（`_tag` 判別子）                | 全 VO に導入済み                   | `domain/*/valueObjects/*`                        |
| Result で失敗を型に出す                    | 全 VO と 7 usecase                 | `utils/result`, `usecases/*`                     |
| 状態を判別可能ユニオンで表現               | ArtistProfile の Draft / Published | `domain/artistProfiles/entities`                 |
...
| VO の失敗表現      | Result                   | Result                   | Result                          |
| usecase の失敗表現 | Result                   | —                        | Result                          |

残る不揃いは **エンティティ表現の軸だけ**（`behaviors` と `operations` の統一方針）。
これはカプセル化（getter）を捨てるかどうかの判断を含むため、単独で合意が必要。

`usecases/users/getMe` と `usecases/linkTypes/listLinkTypes` は Result ではないが、
どちらも**失敗経路を持たない**ため対象外とする（`registered: false` は正当な業務結果で
あってエラーではなく、`listLinkTypes` は `findAll` して返すだけ）。
`Result<T, never>` にしても呼び出し側が分岐できないため、記述量だけが増える。
