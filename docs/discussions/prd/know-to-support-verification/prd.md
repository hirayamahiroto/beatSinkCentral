...
| ID  | 施策                                                            | 対応仮説 | ベータでの実装度                         |
| --- | --------------------------------------------------------------- | -------- | ---------------------------------------- |
| F1  | Story を 3 つの問いで構造化（始まり／転機／何を表現したいのか） | H1       | 必須。`story: string` → `StoryChapter[]` |
...
| 5   | いま／一つの行動 | オファーの理由文＋ボタン 1 つ。オファー無しの期間の主 CTA は**フォロー（次の告知を受け取る）＞共有＞動画（聴きどころ）**の優先順位（T00 確定。H5 の戻り道を主に据える） | F2   |
...
- 告知経由（`from=announce`）の訪問者向けの前室（ストーリーズ形式）は**ベータでは作らない**（T00 確定）。ページ本体はスクロールで読める形のみとする（SEO・主催者向け）。
...
| `listening_point_play`          | 聴きどころの再生 | `position`                                                    | H3（転換に効いたのが Story か翻訳か音かを offer_click の前段行動として分解観察） |
...
- 自分のコンセプトを言葉にできているか・したいと思うか（T00 追加。§2 の課題再定義そのものを検証）
- 過去のパフォーマンスが消えていくことを損失と感じているか（T00 追加。同上）
...
T00（2026-09-02）で以下は確定した:

- ~~オファーが無い期間の主 CTA（フォロー／共有／動画）の優先順位~~ → フォロー＞共有＞動画（§5-2）
- ~~告知経由の訪問者向けの前室（ストーリーズ形式）を置くか~~ → ベータでは作らない（§5-2）
- ~~`profile-information-design.md` §6 の「活動コンセプトを Story に内包するか」~~ → Story 第 3 問「何を表現したいのか（コンセプト）」に内包する（F1）

引き続き未決（ベータの実装をブロックしない）:

- 翻訳段落を誰が書くか（運営／シーンの先輩／本人同士）の**長期**方針。ベータ中は運営が書く（F3・§7-6）ことは確定済み。編集力を moat にするかの分岐は成立後に判断
- 収益の賭け先（取引／編集／SaaS）。本書は取引を主・編集を手段として仮置き。§8 の結果で確定
- 「言語化の課題」（本人がコンセプトを言葉にできていない可能性。§2 課題の2層化のさらに手前）の定量検証案。[`concept-and-loops.md`](./concept-and-loops.md) §2・§6 項目5（未合意、2026-09 追加）
