...
- 画像はプロフィール編集からの **Supabase Storage への直接アップロード**に移行（`POST /api/artists/me/profile/image`、方式は [`../architecture/server/database/storage.md`](../architecture/server/database/storage.md)）。サーバー側（API・バケット）は対応済み。UI の URL 入力をファイルアップロードに置き換えるクライアント側は後続 PR で対応
