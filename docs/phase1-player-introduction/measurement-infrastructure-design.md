...
> [`profile-information-design.md`](./profile-information-design.md) §5-2 の**計測の意図**を起点に、「どうデータを貯め／どう計測を実装するか」を調査・提案する。**WIP**。一部は現行規範（[`profile-api-design.md`](./profile-api-design.md) §5）と意図的に分岐する（→ §6）。
...
- migration は [`database-migration.md`](../server-architecture/database-migration.md) の手順（`db:generate`→`db:migrate`）。
...
- [`profile-information-design.md`](./profile-information-design.md) §5-2（上流）/ [`profile-api-design.md`](./profile-api-design.md) §5（分岐元）
- [`../frontend-architecture/bff.md`](../frontend-architecture/bff.md) / [`../server-architecture/database-migration.md`](../server-architecture/database-migration.md)
