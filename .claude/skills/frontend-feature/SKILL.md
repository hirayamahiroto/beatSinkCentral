...
`tsc` / lint / test は構造適合しか見ない。`.claude/rules/code-review-checklist.md` の🔴🟡項目の多くは
意味的な判断（DBマスタ参照の妥当性・表示語彙の出所・レイヤー境界）で、機械検証だけでは検知できない。

**`Agent` を1本立てて diff を敵対的にレビューさせる。** 自分の実装を自分で見ると、意図が見えているぶん盲点が残る。
`code-review-checklist.md` を照らし合わせて実行する。BFF route（データ取得・整形）を変更した場合は §9 レイヤー境界・
§14 表示語彙のハードコードを重点的に見る。

**指摘が出たらそこで終わらない。** 修正し、Step 4（機械検証）から再実行する。レビューして終わりにせず、
green になるまでこのループを回してから Step 5 へ進む。
