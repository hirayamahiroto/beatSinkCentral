...
横断のレビュー観点は `code-review-checklist` Skill（Step 4b で必ず呼ぶ）。
...
2. Skill 内の「レイヤー別の適用マップ」から、**この変更が触れたレイヤーの §** を選ぶ（BFF route / ClientAdapter・hook / packages/ui の行が主に該当する）。
...
`tsc` / lint / test は構造適合しか見ない。`code-review-checklist` の🔴🟡項目の多くは
意味的な判断（DBマスタ参照の妥当性・表示語彙の出所・レイヤー境界）で、機械検証だけでは検知できない。
...
BFF route（データ取得・整形）を変更した場合は §9 レイヤー境界・§14 表示語彙のハードコードを重点的に見る。
