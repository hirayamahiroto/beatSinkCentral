...
description: プロジェクト共通のコード品質基準（N+1・過剰取得・スコープ条件・権限露出・トランザクション境界・型安全な Optional・コメント方針・マスタ参照・テストの検知力等）を、変更差分に照らして判定・実行するチェックリスト。api-server-feature / frontend-feature の実装完了後に必ず呼ぶ。実装コードを書いた直後・レビューするときに使う。
---
...
`api-server-feature` / `frontend-feature` の実装完了後（Step 4b）に **必ず** 呼ばれる。単体でレビュー依頼を受けたときも同じ手順で使う。

1. **対象差分を確定する** — `git diff --stat main...HEAD`（または指示された範囲）で変更ファイルを列挙する。
2. **該当する § を選ぶ** — 下の「レイヤー別の適用マップ」で、変更したレイヤーに紐づく § を洗い出す。触れていないレイヤーの § は対象外にしてよい。
3. **§ ごとに判定する** — 各 § の本文とチェックポイントを差分に照らし、`該当なし` / `準拠` / `違反` のいずれかに判定する。`違反` は該当箇所（`path:line`）と理由を添える。
4. **🔴 は修正してから先へ進む** — ブロッキング項目の `違反` は、報告だけで終えず修正する。🟡 は修正するか、外れる理由を報告に書く。⚪ は判断を報告に残す。
5. **結果を表で報告する** — 呼び出し元の Skill の「出力」に、次の形で組み込む。

| §   | 優先度 | 判定        | 根拠（path:line / 理由） |
| --- | ------ | ----------- | ------------------------ |
| 3   | 🔴     | 準拠        | …                        |
| 10  | 🔴     | 違反→修正済 | …                        |
| 1   | 🟡     | 該当なし    | クエリ変更なし           |
...
| 変更したレイヤー                                          | 必ず判定する §                                                                                          |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| repository / クエリ（api-server）                         | §1 N+1 / §2 過剰取得 / §3 スコープ条件 / §7 生SQL / §8 SQL とアプリの責務分離                           |
| usecase / policy / capabilities / authorization           | §3 スコープ条件 / §4 権限露出 / §9 レイヤー境界 / §10 トランザクション境界                              |
| route / errorMap / ログ出力                               | §4-3 ログマスク / §9 レイヤー境界 / §12 Optional フォールバック                                         |
| DB スキーマ / migration                                   | §14-1 マスタ参照                                                                                        |
| Entity / VO / export の追加                               | §6-1 呼び手のない公開面（knip / lint の結果を添える）                                                   |
| BFF route（beatfolio）                                    | §2 過剰取得 / §9 レイヤー境界 / §11 インターフェース設計 / §12 Optional フォールバック / §14-2 表示語彙 |
| ClientAdapter / hook / state                              | §9-3 ローカル状態と副作用 / §11-3 非同期データの初期値 / §12 Optional フォールバック                    |
| packages/ui コンポーネント                                | §9-3 ローカル状態と副作用 / §14-2 表示語彙のハードコード                                                |
| テスト（request/response・引数のフィールド変更を含む PR） | §15 テストの検知力                                                                                      |
| すべての変更                                              | §13 コメント方針 / §12 Optional フォールバック                                                          |
...
> 設計思想の本体は [`docs/architecture/server/database/design.md`](../../../docs/architecture/server/database/design.md)（canonical）。本項はそのレビュー観点版。
