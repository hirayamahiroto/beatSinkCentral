...
const { deps, artistProfiles, offers, resolveActorState } =
...
describe("GET /artists/:artistId/profile", () => {
  ...
  it("Actor と一致する artistId なら集約の構造と公開可能性を返す", async () => {
    ...
    expect(artistProfiles.load).toHaveBeenCalledWith("artist-1");
  });
  ...
  it("開催日前のオファーがあれば offer として集約と同じ応答に載せる", async () => {
    ...
    expect(offers.findLatestByArtistId).toHaveBeenCalledWith("artist-1");
  });
  ...
  it("Actor と一致しない artistId は 404 を返し、プロフィールを読まない", async () => {
    ...
    expect(artistProfiles.load).not.toHaveBeenCalled();
    expect(offers.findLatestByArtistId).not.toHaveBeenCalled();
  });
