...
const request = (file: File | string) => {
  ...
  return createApp("auth0|123").request("/", {
    method: "POST",
    body: form,
  });
};
...
describe("POST /artists/me/profile/image", () => {
  ...
  it("画像をアップロードし imageUrl を返す", async () => {
    const res = await request(createImageFile());
  ...
  it("サポート外の contentType は 422 を返し、アップロードしない", async () => {
    const res = await request(createImageFile({ type: "image/gif" }));
  ...
  it("サイズ上限超過は 413 を返し、アップロードしない", async () => {
    const res = await request(createImageFile({ size: 5 * 1024 * 1024 + 1 }));
  ...
  it("空ファイルは 422 を返し、アップロードしない", async () => {
    const res = await request(createImageFile({ size: 0 }));
